"""
Debug script: single ruling — runs search + Auditor on one hardcoded
query and prints the per-article checklist, applicability decision, and
any API/parse errors in full detail.

The annotation file has NOT been restructured to gold_checklist_by_article
yet, so the gold checklist for the target ruling is embedded directly in
this script (see _GOLD_CHECKLIST_BY_ARTICLE below). Swap _TARGET_RULING_ID
and _GOLD_CHECKLIST_BY_ARTICLE to debug a different case.
...
# Manually restructured from this ruling's flat gold_checklist, per-article.
# Replace with a real load from gold_checklist_by_article once that file
# exists — see the "harness note" printed at the end of run().
_GOLD_CHECKLIST_BY_ARTICLE: dict[str, list[dict]] = {
    "قانون مجازات اسلامی - ماده ۶۴۱": [
        {"condition": "حداکثر مجازات قانونی بزه مزاحمت تلفنی طبق این ماده شش ماه حبس است", "necessary": True, "satisfied": True},
    ],
    "قانون مجازات اسلامی - ماده ۶۶": [
        {"condition": "تکلیف دادگاه به تعیین مجازات جایگزین حبس به جای حبس طبق این ماده", "necessary": True, "satisfied": True},
    ],
    "قانون آیین دادرسی کیفری - ماده ۴۷۴": [
        {"condition": "انطباق مورد با بند چ این ماده (عدم تعیین مجازات جایگزین حبس در جرایم با حداکثر شش ماه حبس) برای تجویز اعاده دادرسی", "necessary": True, "satisfied": True},
        {"condition": "ارائه دلیل یا مدرک جدید مؤثر که تجویز اعاده دادرسی از حیث ماهیت اتهام را ایجاب کند (بند دیگر همین ماده)", "necessary": False, "satisfied": False},
    ],
}
...
def run() -> None:
    ...
    from apps.search.calibration.leakage import exclude_self_ruling
    ...
    provider = getattr(django_settings, "AUDITOR_LLM_PROVIDER", "groq")
    ...
    for i, (ref, tier, weight) in enumerate(ranked, start=1):
        ...
        gold_mark = "  [GOLD]" if ref in _GOLD_CHECKLIST_BY_ARTICLE else ""
        kept = "IN " if i <= max_articles else "OUT"
        print(f"  {i:>3}. [{kept}] [{tier:<8}] weight={weight:.4f}  {ref}{gold_mark}{cutoff_mark}")
 
    gold_refs_debug = set(_GOLD_CHECKLIST_BY_ARTICLE)
    ...
    for bundle in bundles:
        if bundle.article_ref not in _GOLD_CHECKLIST_BY_ARTICLE:
            continue
        print(f"\n[{bundle.article_ref}]")
    ...
    print(f"\nRunning Auditor via provider={provider!r}: "
    ...
    for i, bundle in enumerate(bundles):
        ...
        try:
            checklist = verify_article(target.query, bundle)
            ...
            continue
        audited_by_ref[bundle.article_ref] = decide(bundle.article_ref, checklist)
    ...
    for ref in search_result.article_refs:
        ...
        gold_items = _GOLD_CHECKLIST_BY_ARTICLE.get(ref)
 
        print(f"\n[{ref}]")
        if ref not in _GOLD_CHECKLIST_BY_ARTICLE:
        ...
        if audited is None:
            if ref in errored_refs:
                ...
                if ref in _GOLD_CHECKLIST_BY_ARTICLE:
                    print("     ⚠ this IS a gold article and got cut by the ranking/cap -- "
                          "raise AUDITOR_MAX_ARTICLES_PER_QUERY or fix the ranking")
            continue
 
        print(f"  is_applicable={audited.is_applicable}  auditor_confidence={audited.auditor_confidence}")
    ...
    gold_refs = set(_GOLD_CHECKLIST_BY_ARTICLE)
    ...
    print(
        "\n  harness note: gold checklist here is hardcoded in this script for "
        "one ruling. Once apps/search/calibration/data restructuring produces "
        "a real gold_checklist_by_article.json, replace _GOLD_CHECKLIST_BY_ARTICLE "
        "with a lookup from that file."
    )
