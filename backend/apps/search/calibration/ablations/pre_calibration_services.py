...
_FEATURE_MIN_SCORE = 0.80  # calibrated from observed gap: relevant ~0.83, noise ~0.76-0.77
...
def _search_features(session, query: str, embedding: list[float], top_k: int) -> list[Evidence]:
    ...
    (high precision, no similarity threshold needed). Falls back to
    embedding similarity ONLY if no vocabulary term matched — short legal
    terms don't separate reliably in embedding space (see توقیف/توقف,
    gap of 0.043 with no viable threshold).
    """
    ...
    # Fallback: embedding similarity, only when no vocabulary term matched
...
def _search_features_embedding_fallback(session, embedding: list[float], top_k: int) -> list[Evidence]:
    """Original embedding-based search — kept only as fallback now."""
    results = []
    seen: set[str] = set()
...
class SearchService:
    ...
    def search(
        ...
        with neo4j_client.session() as session:
            feature_results = (
                _search_features(session, routing.rewritten_query, embedding, feature_top_k)
